...
type ClassifierRow = (String, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32);
    ...
    for activity in activities {
        ...
        for chunk in remaining.chunks_exact(30) {
            ...
            let vector: Vec<f32> = if opt.without_input_gyro {
                ...
                chunk.iter().flat_map(|v| vec![v.1, v.2, v.3, v.4, v.5, v.6,v.7]).collect()
            } else if opt.without_acceleration {
                chunk.iter().flat_map(|v| vec![v.4, v.5, v.6,v.7]).collect()
            } else {
                chunk.iter().flat_map(|v| vec![v.1, v.2, v.3, v.4, v.5, v.6,v.7, v.8, v.9, v.10]).collect()
            };
            if name == "push" {
